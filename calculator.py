...
class UtilityCalculator:
    ...
    def __init__(self):
        ...
        # EV Data: list of dicts { 'users': [indices], 'kwh': float, 'rate': float }
        self.ev_usage: List[Dict[str, Any]] = [] 

    def load_data(self, data: Dict[str, Any]):
        """
        Loads data from a dictionary (e.g. from bills.py).
        """
        self.residents = data.get('residents', [])
        self.costs = data.get('costs', self.costs)
        self.ev_usage = data.get('ev_usage', [])
        print("Data loaded into Calculator.")

    def interactive_setup_residents(self):
        """Manually configure residents via CLI."""
        count = get_int_input("Enter number of residents: ")
        for i in range(count):
            name = input(f"Enter name for Resident {i+1}: ")
            self.residents.append(Resident(name))
        print(f"--- Configured {len(self.residents)} residents ---")

    def interactive_input_costs(self):
        """Manually input bill totals."""
        print("\n--- Enter Monthly Bill Totals ---")
        self.costs['water'] = get_float_input("Water Meter Cost: $")
        self.costs['waste'] = get_float_input("Waste Meter Cost: $")
        self.costs['storm'] = get_float_input("Storm Waste Cost: $")
        self.costs['transport'] = get_float_input("Transportation Cost: $")
        self.costs['electric_total'] = get_float_input("Total Electric Bill Amount: $")
    ...
    def interactive_input_ev(self):
        ...
        for i in range(num_ev_groups):
            ...
            # Select residents
            print("Select residents who share this EV (enter indices separated by comma):")
